          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {produtos.map((p: any) => {
                    const isBaixo = Number(p.estoque) < Number(p.estoque_minimo ?? 0);
                    return (
                      <tr key={p.id} className="border-b cursor-pointer hover:bg-muted/40" onClick={() => openView(p)}>
                        ...
                        <td className="px-4 py-2.5 text-right tabular">
                          <span className={isBaixo ? "font-semibold text-destructive" : "font-medium"}>{Number(p.estoque)} {p.unidade}</span>
                        </td>
                        ...
                        <td className="px-4 py-2.5">
                          {isBaixo ? <StatusBadge status="crítico" tone="danger" /> : <StatusBadge status="ok" tone="success" />}
                        </td>
                  ...
                  <div key={p.id} className="flex items-start gap-3 p-3">
                    ...
                    <button onClick={() => openView(p)} className="min-w-0 flex-1 text-left">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-xs">
                        <span className={isBaixo ? "font-semibold text-destructive tabular" : "tabular"}>{Number(p.estoque)} {p.unidade}</span>
                        <span className="text-muted-foreground">·</span>
                        <span className="tabular">{formatBRL(Number(p.preco_venda))}</span>
                        {isBaixo && <StatusBadge status="baixo" tone="danger" />}
                      </div>
                    </button>
